	...
	while(fgets(line,LIN,fpin)){ 
		if(!strncmp(line,"ATOM",4) 
		 ...
		 ||!strncmp(&line[16]," ",1))){
			...
			pdb->TypeAtomId[i]=Atom2Int2(pdb->TypeAtom[i]);			
...
int readpdbModel(PDB *pdb, char *filename, int n,int mid){
...
//start from 0
int CountAtomModel(char *filename,int mid){
	int i,j; 
	FILE *fpin; 
	char line[LIN], buf[LIN]; 
	double x,y,z;
...
int AA2int(char *TypeAtom){
        ...
        if(!strcmp(TypeAtom,"UNK")){return 20;}
        return -1;
}
