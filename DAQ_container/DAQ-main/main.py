...
if __name__ == "__main__":
    ...
    if params['mode']==0:
        ...
        cur_map_name = params["F"].split("/")[-1]
        cur_map_path = os.path.abspath(os.path.join("./inputs", cur_map_name))

        final_dst = os.path.abspath(os.path.join("./inputs", cur_map_name.split(".")[0]))
        ...
        if model_path != "/DAQ-main/best_model/qa_model/Multimodel.pth":
            model_name = params["M"].split("/")[-1]
            model_path = os.path.abspath(os.path.join("./inputs", model_name))


        # pdb_path = os.path.abspath(params['P'])
        pdb_name = params["P"].split("/")[-1]
        pdb_path = os.path.abspath(os.path.join("./inputs", pdb_name))
        ...
        os.system(f"cp -r {save_path} {final_dst}")
    elif params['mode']==1:
        ...
        pdb_name = params["P"].split("/")[-1]
        pdb_dir = os.path.abspath(os.path.join("./inputs", pdb_name))
